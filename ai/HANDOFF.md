...
- 新增 `POST /api/track` 前端漏斗埋点:事件白名单 `ids_input`(首次动输入框)/`submit_click`(带条目数、前 5 条输入样本、是否自带 key、选项)/`client_blocked`(拦截原因 + 原始输入前 300 字)/`client_error`(错误码+信息);body 上限 4KB,meta 超 1200 字符截断,异常静默。
- `/stats` 总览卡片扩成 8 个:页面访问/独立访客/动过输入框/点击生成/前端拦截/前端报错/推荐执行/推荐成功,一眼看漏斗断在哪层;事件详情表可见被拦的原始输入。
- 已部署 + 推 GitHub。待新流量进来后看漏斗数据再定下一步(候选方向:示例小队一键试用、把"怎么拿好友链接"写得更傻瓜、好友列表 API 自动拉队友)。
...
- `FIT_COSINE_CAP=0.55` 等系数用用户真实小队(3 人,重度生存建造口味)验证过一轮,推荐梯度 72–98 合理;但该小队口味高度集中,口味分散的小队还没验证过,系数可能仍需微调。
- 用户小队实测:腐蚀(138 万评测)凭 ~1.0 的相似度扛着 ×0.55 降权仍排第一——对这桌人其实是合理推荐,暂不加码;如果用户反馈还是太大路货,下一步再调。
